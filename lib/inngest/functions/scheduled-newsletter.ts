...
export default inngest.createFunction(
    ...
    async ({event, step, runId}) => {
        try {
            ...
            if (!isUserActive) {
                console.log(`User ${event.data.userId} has paused their newsletter. Skipping processing.`);
                return {
                    skipped: true,
            ...
            const allArticles = await step.run("fetch-news", async () => {
                console.log(`Fetching articles for categories: ${event.data.categories.join(", ")}`);
            ...
            if (!event.data.isTest) {
                await step.run("schedule-next", async () => {
                    ...
                    console.log(`Next newsletter scheduled for: ${nextScheduleTime.toISOString()}`);
                });
            }
